...
const STATE_DIR = process.env.CHEER_STATE_DIR || path.join(CLAUDE_DIR, 'state');
const QUEUE_DIR = path.join(STATE_DIR, 'cheer_queue');
// ponytail: 同一個 env var 兼管 state 與 log，測試設 CHEER_STATE_DIR 就整組隔離
const LOG_DIR = process.env.CHEER_STATE_DIR || path.join(CLAUDE_DIR, 'logs');
